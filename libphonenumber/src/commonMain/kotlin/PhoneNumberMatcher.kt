...
import PhoneNumberUtil.Companion.formattingRuleHasFirstGroupOnly
import PhoneNumberUtil.Companion.normalizeDigits
import PhoneNumberUtil.Companion.normalizeDigitsOnly
import PhoneNumberUtil.Leniency
...
import libphonenumber.util.currencySymbols
...
class PhoneNumberMatcher(
    ...
    */
    ...
    private fun find(index: Int): PhoneNumberMatch? {
        return coroutineScope.async {
            var index = index
            var match: PhoneNumberMatch? = null
            while (maxTries > 0) {
    ...
    */
    private suspend fun extractMatch(candidate: CharSequence, offset: Int): PhoneNumberMatch? {
    ...
    * @return  the match found, null if none can be found
    */
    private suspend fun extractInnerMatch(candidate: CharSequence, offset: Int): PhoneNumberMatch? {
    ...
    */
    private suspend fun parseAndVerify(candidate: CharSequence, offset: Int): PhoneNumberMatch? {
        try {
    ...
    */
    ...
    fun checkNumberGroupingIsValid(
        ...
        return coroutineScope.async {
            ...
            var bool = checker.checkGroups(util, number, normalizedCandidate, formattedNumberGroups)
            if (bool) {
    ...
    * formats use spaces to separate each digit. */
        ...
        fun allNumberGroupsRemainGrouped(
            ...
            for (i in formattedNumberGroups.indices) {
                ...
                if (i == 0 && fromIndex < normalizedCandidate.length) {
                    ...
                    val string = util.coroutineScope.async {
                        util.getNddPrefixForRegion(region, true)
                    }.getCompleted()
                    if (string != null && normalizedCandidate[fromIndex].isDigit()) {
        ...
        */
        private suspend fun getNationalNumberGroups(util: PhoneNumberUtil, number: PhoneNumber): Array<String> {
        ...
        */
        ...
        suspend fun containsOnlyValidXChars(
        ...
        suspend fun isNationalPrefixPresentIfRequired(number: PhoneNumber, util: PhoneNumberUtil): Boolean {
