...
class MetadataSourceImpl(
    ...
    constructor(
        ...
        BlockingMetadataBootstrappingGuard<CompositeMetadataContainer>(
            metadataParser!!, CompositeMetadataContainer(),
            coroutineScope
        ),
        coroutineScope
    )

    override suspend fun getMetadataForNonGeographicalRegion(countryCallingCode: Int): PhoneMetadata? {
        require(!isGeoEntity(countryCallingCode)) { "$countryCallingCode calling code belongs to a geo entity" }
        return bootstrappingGuard.getOrBootstrap(
            phoneMetadataResourceProvider.getFor(
                countryCallingCode
            )
        )
