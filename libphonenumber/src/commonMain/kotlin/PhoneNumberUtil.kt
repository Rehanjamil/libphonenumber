...
import libphonenumber.PhoneNumberMatcher.NumberGroupingChecker
import libphonenumber.Phonenumber.PhoneNumber
import libphonenumber.Phonenumber.PhoneNumber.CountryCodeSource
import libphonenumber.internal.RegexBasedMatcher.Companion.create
import libphonenumber.internal.RegexCache
import libphonenumber.metadata.DefaultMetadataDependenciesProvider
import libphonenumber.metadata.source.MetadataSourceImpl
import libphonenumber.util.InplaceStringBuilder
        ...
        STRICT_GROUPING {
            override suspend fun verify(
                ...
                return if ((!util.isValidNumber(number) || !PhoneNumberMatcher.containsOnlyValidXChars(
                    ...
                    matcher.checkNumberGroupingIsValid(
                        number,
                        candidate,
                        util,
...
//    }
     ...
     */
    suspend fun getLengthOfNationalDestinationCode(number: PhoneNumber): Int {
     ...
     * be non-null.
     */
    private fun getSupportedTypesForMetadata(metadata: Phonemetadata.PhoneMetadata?): Set<PhoneNumberType> {
        val types: MutableSet<PhoneNumberType> = mutableSetOf()
        for (type in PhoneNumberType.entries) {
     ...
     */
    suspend fun getSupportedTypesForRegion(regionCode: String): Set<PhoneNumberType> {
        if (!isValidRegionCode(regionCode)) {
            println("Invalid or unknown region code provided: $regionCode")
            return setOf()
        }
     ...
     * entity.
     */
    suspend fun getSupportedTypesForNonGeoEntity(countryCallingCode: Int): Set<PhoneNumberType> {
     ...
     */
    suspend fun isNumberGeographical(phoneNumber: PhoneNumber): Boolean {
    ...
    fun format(
        ...
        val metadata = coroutineScope.async {
        ...
        if (formattingPattern == null) {
            ...
            if (nationalPrefixFormattingRule.length > 0) {
                ...
                if (nationalPrefix.length > 0) {
                    ...
                    nationalPrefixFormattingRule =
                        nationalPrefixFormattingRule.replace(FG_STRING, "$1")
     ...
     */
    suspend fun formatNationalNumberWithCarrierCode(
        number: PhoneNumber,
    ...
    private suspend fun getMetadataForRegionOrCallingCode(
        countryCallingCode: Int, regionCode: String?
    ): Phonemetadata.PhoneMetadata? {
        return if (REGION_CODE_FOR_NON_GEO_ENTITY == regionCode) getMetadataForNonGeographicalRegion(
            countryCallingCode
        ) else getMetadataForRegion(
            regionCode
        )
    }
     ...
     * none is found
     */
    suspend fun formatNationalNumberWithPreferredCarrierCode(
     ...
     * @return  the formatted phone number
     */
    suspend fun formatNumberForMobileDialing(
        ...
        if ((regionCallingFrom == regionCode)) {
            ...
            // dialled.
            return if (withFormatting) format(
                numberNoExt,
     ...
     * @return  the formatted phone number
     */
    suspend fun formatOutOfCountryCallingNumber(
        ...
        if (countryCallingCode == NANPA_COUNTRY_CODE) {
            if (isNANPACountry(regionCallingFrom)) {
                ...
                return countryCallingCode.toString() + " " + format(
                    number,
     ...
     */
    suspend fun formatInOriginalFormat(number: PhoneNumber, regionCallingFrom: String): String? {
...
//                formatOutOfCountryCallingNumber(number, regionCallingFrom)
    ...
    private suspend fun rawInputContainsNationalPrefix(
    ...
    private suspend fun hasFormattingPatternForNumber(number: PhoneNumber): Boolean {
        val countryCallingCode = number.countryCode
        val phoneNumberRegion = getRegionCodeForCountryCode(countryCallingCode)
        val metadata =
            getMetadataForRegionOrCallingCode(countryCallingCode, phoneNumberRegion) ?: return false
        val nationalNumber = getNationalSignificantNumber(number)
     ...
     * @return  the formatted phone number
     */
    suspend fun formatOutOfCountryKeepingAlphaChars(
        number: PhoneNumber, regionCallingFrom: String
    ): String {
    ...
    private fun formatNsn(
        number: String,
        metadata: Phonemetadata.PhoneMetadata?,
        numberFormat: PhoneNumberFormat,
    ...
    fun chooseFormattingPatternForNumber(
        ...
        for (numFormat in availableFormats) {
            ...
            if (size == 0 || regexCache.getRegexForPattern( // We always use the last leading_digits_pattern, as it is the most detailed.
                ...
                if (regexCache.getRegexForPattern(numFormat.pattern)
                        .matchEntire(nationalNumber) != null
                ) {
                    return numFormat
                }
            }
    ...
    private fun formatNsnUsingPattern(
        ...
        if (numberFormat == PhoneNumberFormat.NATIONAL && carrierCode != null && carrierCode.length > 0 && formattingPattern.domesticCarrierCodeFormattingRule.length > 0) {
            ...
            carrierCodeFormattingRule =
                carrierCodeFormattingRule.replace(CC_STRING, carrierCode.toString())
            // Now replace the $FG in the formatting rule with the first group and the carrier code
            // combined in the appropriate way.
            numberFormatRule =
                numberFormatRule.replaceFirst(FIRST_GROUP_PATTERN, carrierCodeFormattingRule)
        ...
        if (numberFormat == PhoneNumberFormat.RFC3966) {
            ...
            if (matchResult != null) {
                formattedNationalNumber =
                    formattedNationalNumber.replaceFirst(SEPARATOR_PATTERN, "")
            }
     ...
     */
    suspend fun getExampleNumber(regionCode: String): PhoneNumber? {
     ...
     */
    suspend fun getInvalidExampleNumber(regionCode: String): PhoneNumber? {
        if (!isValidRegionCode(regionCode)) {
            println("Invalid or unknown region code provided: $regionCode")
            return null
        }
     ...
     */
    suspend fun getExampleNumberForType(regionCode: String, type: PhoneNumberType?): PhoneNumber? {
     ...
     */
    suspend fun getExampleNumberForType(type: PhoneNumberType?): PhoneNumber? {
     ...
     */
    suspend fun getExampleNumberForNonGeoEntity(countryCallingCode: Int): PhoneNumber? {
        val metadata = getMetadataForNonGeographicalRegion(countryCallingCode)
        if (metadata != null) {
            ...
            for (desc: Phonemetadata.PhoneNumberDesc? in listOf(
                metadata.mobile,
                metadata.tollFree,
                metadata.sharedCost,
                metadata.voip,
    ...
    private fun maybeAppendFormattedExtension(
        ...
        if (number.hasExtension() && number.extension.isNotEmpty()) {
            if (numberFormat == PhoneNumberFormat.RFC3966) {
    ...
    fun getNumberDescByType(metadata: Phonemetadata.PhoneMetadata?, type: PhoneNumberType?): Phonemetadata.PhoneNumberDesc? {
        return when (type) {
     ...
     */
    suspend fun getNumberType(number: PhoneNumber): PhoneNumberType {
        val regionCode = getRegionCodeForNumber(number)
        val metadata =
            getMetadataForRegionOrCallingCode(number.countryCode, regionCode)
                ?: return PhoneNumberType.UNKNOWN
        val nationalSignificantNumber = getNationalSignificantNumber(number)
        return getNumberTypeHelper(nationalSignificantNumber, metadata)
    }

    private fun getNumberTypeHelper(
        nationalNumber: String,
        metadata: Phonemetadata.PhoneMetadata?
    ): PhoneNumberType {
        ...
        return if ((!metadata.sameMobileAndFixedLinePattern && isNumberMatchingDesc(
                nationalNumber,
                metadata.mobile
            ))
        ) {
     ...
     */
    suspend fun getMetadataForRegion(regionCode: String?): Phonemetadata.PhoneMetadata? {
        if (!isValidRegionCode(regionCode)) {
            return null
        }
        val phoneMetadata = metadataSource.getMetadataForRegion(regionCode, coroutineScope)
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    suspend fun getMetadataForNonGeographicalRegion(countryCallingCode: Int): Phonemetadata.PhoneMetadata? {
        var phoneMetadata: Phonemetadata.PhoneMetadata? = null
        phoneMetadata = metadataSource.getMetadataForNonGeographicalRegion(
            countryCallingCode
        )
    ...
    fun isNumberMatchingDesc(nationalNumber: String, numberDesc: Phonemetadata.PhoneNumberDesc?): Boolean {
     ...
     */
    suspend fun isValidNumber(number: PhoneNumber): Boolean {
        val regionCode = getRegionCodeForNumber(number)
     ...
     */
    @OptIn(ExperimentalCoroutinesApi::class)
    suspend fun isValidNumberForRegion(number: PhoneNumber, regionCode: String?): Boolean {
        val countryCode = number.countryCode
        ...
        if (
            ...
            val bool = countryCode != getCountryCodeForValidRegion(
                regionCode
            )
            if (bool) {
                return false
    ...
    fun getRegionCodeForNumber(number: PhoneNumber): String? {
        ...
        if (regions == null) {
            println("Missing/invalid country_code ($countryCode)")
            return null
        }
    ...
    private suspend fun getRegionCodeForNumberFromRegionList(
     ...
     */
    suspend fun getCountryCodeForRegion(regionCode: String?): Int {
        if (!isValidRegionCode(regionCode)) {
            println(
    ...
    private suspend fun getCountryCodeForValidRegion(regionCode: String?): Int {
        val metadata =
            getMetadataForRegion(regionCode)
        return metadata?.countryCode ?: 0
    }
     ...
     */
    suspend fun getNddPrefixForRegion(regionCode: String?, stripNonDigits: Boolean): String? {
        val metadata = getMetadataForRegion(regionCode)
        if (metadata == null) {
            return null
        }
     ...
     */
    suspend fun isPossibleNumber(number: PhoneNumber): Boolean {
     ...
     */
    suspend fun isPossibleNumberForType(number: PhoneNumber, type: PhoneNumberType): Boolean {
    ...
    private fun testNumberLength(
        number: CharSequence,
        metadata: Phonemetadata.PhoneMetadata?,
        type: PhoneNumberType = PhoneNumberType.UNKNOWN
    ): ValidationResult {
     ...
     */
    suspend fun isPossibleNumberWithReason(number: PhoneNumber): ValidationResult {
     ...
     * @return  a ValidationResult object which indicates whether the number is possible
     */
    suspend fun isPossibleNumberForTypeWithReason(
     ...
     * @return  true if the number is possible
     */
    suspend fun isPossibleNumber(number: CharSequence?, regionDialingFrom: String?): Boolean {
        return try {
     ...
     */
    suspend fun truncateTooLongNumber(number: PhoneNumber): Boolean {
    ...
    fun getAsYouTypeFormatter(regionCode: String): AsYouTypeFormatter {
        ...
        return AsYouTypeFormatter(this, regionCode, coroutineScope = coroutineScope)
    ...
    fun extractCountryCode(
        fullNumber: InplaceStringBuilder,
    ...
    fun maybeExtractCountryCode(
        number: CharSequence,
        defaultRegionMetadata: Phonemetadata.PhoneMetadata?,
        nationalNumber: InplaceStringBuilder,
        ...
        if (countryCodeSource !== CountryCodeSource.FROM_DEFAULT_COUNTRY) {
            ...
            if (normalizedNumber.startsWith(defaultCountryCodeString)) {
                ...
                maybeStripNationalPrefixAndCarrierCode(
                    ...
                    null /* Don't need the carrier code */
                )
    ...
    fun maybeStripNationalPrefixAndCarrierCode(
        number: InplaceStringBuilder, metadata: Phonemetadata.PhoneMetadata, carrierCode: InplaceStringBuilder?
    ): Boolean {
        ...
        if (prefixMatchResult != null) {
            ...
            val isViableOriginalNumber =
                matcherApi.matchNationalNumber(number, generalDesc!!, false)
            ...
            return if (transformRule.isEmpty() || prefixMatchResult.groups[numOfGroups] == null) {
                ...
                transformedNumber.replaceRange(
                    0,
                    numberLength,
                    number.replaceFirst(prefixMatchRegex, transformRule)
                )
                if (isViableOriginalNumber && !matcherApi.matchNationalNumber(
     ...
     */
    private fun checkRegionForParsing(
        numberToParse: CharSequence?,
        defaultRegion: String?
    ): Boolean {
        if (!isValidRegionCode(defaultRegion)) {
            // If the number is null or empty, we can't infer the region.
            if (numberToParse == null || numberToParse.length == 0 || !PLUS_CHARS_PATTERN.matchesAt(
                    numberToParse,
                    0
                )
            ) {
     ...
     */
    suspend fun parse(numberToParse: CharSequence?, defaultRegion: String?): PhoneNumber {
     ...
     */
    suspend fun parse(numberToParse: CharSequence?, defaultRegion: String?, phoneNumber: PhoneNumber) {
     ...
     */
    suspend fun parseAndKeepRawInput(numberToParse: CharSequence?, defaultRegion: String?): PhoneNumber {
        val phoneNumber = PhoneNumber()
     ...
     */
    suspend fun parseAndKeepRawInput(
     ...
     */
    @OptIn(ExperimentalCoroutinesApi::class)
    @Throws(NumberParseException::class, CancellationException::class)
    private suspend fun parseHelper(
        numberToParse: CharSequence?,
        defaultRegion: String?,
        keepRawInput: Boolean,
        checkRegion: Boolean,
        ...
        if (countryCode != 0) {
            ...
            if (phoneNumberRegion != defaultRegion) {
                // Metadata cannot be null because the country calling code is valid.
                regionMetadata = coroutineScope.async {
                    getMetadataForRegionOrCallingCode(countryCode, phoneNumberRegion)
                }.getCompleted()
            }
        } else {
     ...
     */
    private fun extractPhoneContext(
        numberToExtractFrom: String,
        indexOfPhoneContext: Int
    ): String? {
     ...
     */
    @Throws(NumberParseException::class)
    private fun buildNationalNumberForParsing(
     ...
     */
    suspend fun isNumberMatch(firstNumber: CharSequence?, secondNumber: CharSequence?): MatchType {
        try {
     ...
     * [.isNumberMatch] for more details.
     */
    suspend fun isNumberMatch(firstNumber: PhoneNumber, secondNumber: CharSequence?): MatchType {
            ...
            if (e.errorType === NumberParseException.ErrorType.INVALID_COUNTRY_CODE) {
                ...
                try {
                    return if (firstNumberRegion != UNKNOWN_REGION) {
                        val secondNumberWithFirstNumberRegion =
     ...
     * outside the region
     */
    suspend fun canBeInternationallyDialled(number: PhoneNumber): Boolean {
...
// internationally diallable, and will be caught here.
     ...
     * portability or not
     */
    suspend fun isMobileNumberPortableRegion(regionCode: String): Boolean {
        val metadata = getMetadataForRegion(regionCode)
        if (metadata == null) {
            println("Invalid or unknown region code provided: $regionCode")
            return false
    ...
    companion object {
        ...
        init {
            ...
            GEO_MOBILE_COUNTRIES_WITHOUT_MOBILE_AREA_CODES =
                geoMobileCountriesWithoutMobileAreaCodes
        ...
        private val VALID_ALPHA = (ALPHA_MAPPINGS.keys.toTypedArray().contentToString()
            .replace("[, \\[\\]]".toRegex(), "") + ALPHA_MAPPINGS.keys.toTypedArray()
            .contentToString().lowercase()
            .replace("[, \\[\\]]".toRegex(), ""))
...
//        private const val UNWANTED_END_CHARS = "[[\\P{N}&&\\P{L}]&&[^#]]+$"
        ...
        private fun createExtnPattern(forParsing: Boolean): String {
            ...
            val americanStyleExtnWithSuffix =
                ambiguousSeparator + extnDigits(extLimitWhenNotSure) + "#"
...
//          return VALID_PHONE_NUMBER_PATTERN.matchEntire(number) != null
            val matchLength =
                VALID_PHONE_NUMBER_PATTERN.matchEntire(number)?.groupValues?.first()?.length
        ...
        fun normalize(number: InplaceStringBuilder): InplaceStringBuilder {
            if (VALID_ALPHA_PHONE_PATTERN.matchEntire(number) != null) {
                number.replaceRange(
                    0,
                    number.length,
                    normalizeHelper(number, ALPHA_PHONE_MAPPINGS, true)
                )
            } else {
         ...
         */
        private fun descHasPossibleNumberData(desc: Phonemetadata.PhoneNumberDesc?): Boolean {
         ...
         */
        private fun descHasData(desc: Phonemetadata.PhoneNumberDesc?): Boolean {
         ...
         */
        @JvmStatic
        fun createInstance(coroutineScope: CoroutineScope): PhoneNumberUtil {
            val metadataDependenciesProvider = DefaultMetadataDependenciesProvider(
                coroutineScope
            )
            ...
            return createInstance(metadataSource, metadataDependenciesProvider, coroutineScope)
        }
        ...
        private fun ensureMetadataIsNonNull(phoneMetadata: Phonemetadata.PhoneMetadata?, message: String) {
