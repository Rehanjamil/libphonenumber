...
import PhoneNumberUtil.Companion.normalizeDiallableCharsOnly
...
class AsYouTypeFormatter internal constructor(
    ...
    private var defaultMetadata: Phonemetadata.PhoneMetadata? = null
    private var currentMetadata: Phonemetadata.PhoneMetadata? = null
    ...
    suspend fun finishInitialization(){
    ...
    private suspend fun getMetadataForRegion(regionCode: String): Phonemetadata.PhoneMetadata {
        ...
        while (it.hasNext()) {
            ...
            if (createFormattingTemplate(numberFormat)) {
                ...
                shouldAddSpaceAfterNationalPrefix =
                    NATIONAL_PREFIX_SEPARATORS_PATTERN.containsMatchIn(
    ...
    private fun narrowDownPossibleFormats(leadingDigits: String) {
        ...
        while (it.hasNext()) {
            ...
            val lastLeadingDigitsPattern =
                min(
                    indexOfLeadingDigitsPattern.toDouble(),
                    (format.leadingDigitsPatternCount - 1).toDouble()
                ).toInt()
    ...
    fun inputDigit(nextChar: Char): String {
        currentOutput = coroutineScope.async {
    ...
    private suspend fun inputDigitWithOptionToRememberPosition(
        nextChar: Char,
...
// Numbers beginning with 1[01] can only be short/emergency numbers, which don't need the
     ...
     */
    @OptIn(ExperimentalCoroutinesApi::class)
    private suspend fun attemptToExtractCountryCallingCode(): Boolean {
        return when {
            nationalNumber.isEmpty() -> {
                ...
                val countryCode =
                    phoneUtil.extractCountryCode(nationalNumber, numberWithoutCountryCallingCode)
                when {
                    countryCode == 0 -> {
                        false
                    }
    ...
    private fun normalizeAndAccrueDigitsAndPlusSign(
        nextChar: Char,
        rememberPosition: Boolean
    ): Char {
        ...
        if (rememberPosition) {
            this.positionToRemember = accruedInputWithoutFormatting.length
        }
    ...
    companion object {
        ...
        private val EMPTY_METADATA =
            newBuilder().setId("<ignored>").setInternationalPrefix("NA").build()
